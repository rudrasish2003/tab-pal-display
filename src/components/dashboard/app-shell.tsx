...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <div className={cn("transition-[padding] duration-200", collapsed ? "lg:pl-[68px]" : "lg:pl-[232px]")}>
      ...
      {mobileOpen && <div className="fixed inset-x-0 top-[72px] z-20 border-b border-border bg-sidebar text-sidebar-foreground shadow-lg lg:hidden"><div className="flex items-center justify-between border-b border-sidebar-border px-5 py-4"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-md bg-brand-mark text-brand-mark-foreground"><Landmark className="size-5"/></div><strong>INT</strong></div><Button variant="ghost" size="icon" aria-label="Close navigation" onClick={() => setMobileOpen(false)}><X/></Button></div><div className="py-4"><Navigation onNavigate={() => setMobileOpen(false)}/></div></div>}
